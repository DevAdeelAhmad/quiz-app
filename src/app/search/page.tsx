...
const SearchPage = () => {
    ...
    const handleFilterChange = (name: string, value: string) => {
        setFilters((prevFilters: Record<string, string | null>) => ({ ...prevFilters, [name]: value }));
        ...
        setFilteredQuizzes((prevQuizzes: QuizWithCategory[]) => {
            ...
            const filtered = quizzes.filter((quiz1) => {
                ...
                const difficultyFilter = !filtersToApply.difficulty || quiz1.quizDifficulty.toLowerCase() === filtersToApply.difficulty.toLowerCase();
                const categoryFilter = !filtersToApply.category || quiz1.quizCategory.toLowerCase() === filtersToApply.category.toLowerCase();
                const ratingFilter = filtersToApply.rating === null || quiz1.quizRating === parseInt(filtersToApply.rating, 10);
                const durationFilter =
                    !filtersToApply.duration ||
                    (filtersToApply.duration === 'short' && quiz1.quizDuration < 10) ||
                    (filtersToApply.duration === 'medium' && quiz1.quizDuration >= 10 && quiz1.quizDuration <= 20) ||
                    (filtersToApply.duration === 'long' && quiz1.quizDuration > 20);
                const searchTermFilter =
                    !filtersToApply.searchTerm ||
                    quiz1.quizTitle.toLowerCase().includes(filtersToApply.searchTerm.toLowerCase()) ||
                    quiz1.quizCategory.toLowerCase().includes(filtersToApply.searchTerm.toLowerCase()) ||
                    quiz1.quizTags?.some((tag) => tag.toLowerCase().includes(filtersToApply.searchTerm.toLowerCase())) ||
                    quiz1.quizSubCategory.toLowerCase().includes(filtersToApply.searchTerm.toLowerCase());

                return difficultyFilter && categoryFilter && ratingFilter && durationFilter && searchTermFilter;
            });
        ...
        <main className='flex min-h-screen w-full'>
            ...
            <div className='flex justify-center w-full py-10'>
                <div className="flex flex-col max-w-4xl w-full">
                    ...
                    <div className='py-2'>
                        <Input type='search' placeholder='Search Here...' value={searchTerm} onChange={(e) => handleSearch(e.target.value)} />
                    </div>
                    <Filters filters={filters} onFilterChange={handleFilterChange} onClearFilters={clearFilters} categories={categories?.map(cat => cat.name) || []} />
                    <h1 className='text-xl lg:text-2xl font-semibold'>Search Results ({filteredQuizzes.length})</h1>
