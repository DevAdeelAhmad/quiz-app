...
const SingleMyQuiz = ({ creatorName, quizName, percentage, quizImageUrl, isFavorite }: SingleFeaturedQuizProps) => {
        ...
        <div className='flex flex-col sm:flex-row gap-4 rounded-3xl border hover:shadow-lg p-3 cursor-pointer relative'>
            ...
            <div className="flex flex-col justify-evenly gap-4 pr-10">
                ...
                <div className="flex flex-col gap-2 w-full">
                    <div className={`w-full border bg-gray-300 rounded-xl flex items-center justify-start`}>
                        <div className={`!w-[${percentage}%] p-2 bg-gray-800 rounded-xl`}></div>
                    </div>
                    <span className='text-black/60 text-base'>{percentage}% complete</span>
                </div>
            </div>
        </div >
    )
