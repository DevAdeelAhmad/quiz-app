...
const QuizQuestionUpdate: React.FC<QuizQuestionProps> = ({
  onSubmit,
  existingQuizData,
}) => {
  const [questions, setQuestions] = useState<Question[]>(existingQuizData);
  ...
  useEffect(() => {
    const handleUnload = (event: BeforeUnloadEvent) => {
      if (
        ...
        event.returnValue =
          "All progress will be lost. Are you sure you want to leave?";
      }
    };
  ...
  }, [currentQuestion, currentOptions, questions]);
  useEffect(() => {
    if (showAlert) {
  ...
  const handleAddQuestion = () => {
    if (
      ...
      setAlertMessage(
        "All fields are required. Choose at least 1 correct option."
      );
      setShowAlert(true);
      return;
    }
  ...
  const handleCheckboxChange = (option: string) => {
    if (currentOptions.some((opt) => opt.trim() === "")) {
      setAlertMessage(
        "Please add options before selecting the correct option."
      );
      setShowAlert(true);
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-4 border-2 p-5 rounded-2xl transition-all duration-300 max-w-[800px] w-full max-sm:mt-5">
      <span className="text-center">
        Total Added Questions: {questions.length}
      </span>
      {showAlert && (
      ...
      <div className="w-full flex flex-wrap items-center justify-center gap-5">
        <AlertDialog>
          <AlertDialogTrigger asChild>
            <Button
              ...
              className="bg-dark dark:bg-main text-main dark:text-dark font-semibold !max-w-[100px] py-2"
            >
              Submit
            </Button>
          </AlertDialogTrigger>
        ...
        <Button
          ...
          className="text-white bg-blue-500 py-2"
        >
          Add Question
        </Button>
        <Button
          onClick={handleAddOption}
          disabled={currentOptions.length >= 5}
          className="text-white bg-green-500 py-2"
        >
          Add More Options
        </Button>
      </div>
      <div className="w-full">
        {questions.map((question) => (
          <div
            key={question.id}
            className="flex flex-col gap-2 border-2 p-4 rounded-md"
          >
